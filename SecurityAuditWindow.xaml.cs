...
{
    public partial class SecurityAuditWindow : Window
    {
        public ObservableCollection<SecurityAuditEntry> FilteredAuditEntries { get; } = new();

        private readonly List<SecurityAuditEntry> _allAuditEntries = new();
        ...
        public SecurityAuditWindow(string eventName, DateTime? startDate, DateTime? endDate, string? location, string userFullName, int userId)
        {
            InitializeComponent();
            DataContext = this;
        ...
        private void BtnExport_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var pdfPath = Path.Combine(exportFolder, $"Аудит_СлужбаБезопасности_{timestamp}.pdf");
        ...
        private void ExportToPdf(string pdfPath, List<SecurityAuditEntry> entries)
        {
            QuestPDF.Settings.License = LicenseType.Community;

            var title = string.IsNullOrWhiteSpace(_eventName)
                ? "Аудит службы безопасности"
                : $"Аудит Службы безопасности  — {_eventName}";
        ...
        private void LoadAuditEntries()
        {
            ...
            try
            {
                ...
                const string query = @"SELECT sa.AuditID, sa.AuditDate, sa.Note,
                                               u.LastName, u.FirstName, u.MiddleName
                                        FROM SecurityAudit sa
                                        INNER JOIN Users u ON sa.UserID = u.UserID
                                        ORDER BY sa.AuditDate DESC";
                ...
                while (reader.Read())
                {
                    ...
                    _allAuditEntries.Add(new SecurityAuditEntry
                    {
        ...
        {
            IEnumerable<SecurityAuditEntry> entries = _allAuditEntries;
